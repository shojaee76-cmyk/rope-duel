...
- 2026-09-14 (v3): user asked for MORE SKY and a CLOSER view of the cats. Root cause of the blocked sky: the solid upper wall (y 5.4..8) + tall crenellations. Changes: wall is now a **low parapet** at the arch springline with a stone coping, **engaged piers + imposts** under every arch springing (so the arcade no longer reads as floating hoops), **continuous voussoirs** (per-bay count from arc length, 1.18x overlap - no bead gaps), **arch-masked village vistas** (canvas alpha mask = half-disc + rect, so nothing floats above the parapet), moon lifted 5.9 -> 6.35 so the ring stops bisecting the Bitcoin B, distant sierra retuned as a LOW horizon band (y 5.6-15.6 at z=-54/-46), clouds re-ranged over the whole open sky and de-blown-out (alpha 0.09-0.22, opacity 0.18-0.56). Camera: FOV 45 -> 40, z 11 -> 6.8, lookAt raised to y 3.15, plus a fighter-follow pan (panX = mid*0.42) so a close shot never loses the duel; measured cat height 120px -> **319px (40% of frame height)** via measure.mjs projection. Also: retina 2x pixel ratio (1.25x mobile), contact shadows ON for desktop (PCFSoft, bias -0.0008/normalBias 0.02), moon key light 0.5 -> 0.78, rim lights 30 -> 44. Live verified: bundle byte-identical (sha256 0b7939dce0bf455e), 0 console errors, 58-74 FPS. Tools added: `measure.mjs` (exact on-screen bbox of any scene object via projection), `smoke_v2.mjs` (local render + console/FPS gate), `verify_live_v2.mjs` (same against the deployed URL).
